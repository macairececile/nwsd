import json
from common import get_value_as_int_list, pad_list, get_value_as_str_list
...
class ModelConfig(object):
    ...
    def load_from_file(self, file_path, hf=None):
        file = open(file_path, "r")
        data = json.load(file)
        file.close()
        if hf:
            self.load_from_serializable_data(data, hf)
        else:
            self.load_from_serializable_data(data)        

    def load_from_serializable_data(self, data, hf=None):
        ...
        if hf:
            self.set_input_auto_model(hf, hf)
        else:
