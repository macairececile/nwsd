from common import *
from model import Model, ModelConfig, DataConfig
...
class Predicter(object):
    ...
    def predict(self, to_predict: str, hf=None):
        ...
        config.load_from_file(config_file_path, hf)
        if self.clear_text:
        ...
        if hf:
            ensemble = self.create_ensemble(config, self.ensemble_weights_path, hf)
        else:
        ...
        batch_x = None
        batch_z = None
        # for line in sys.stdin:  # à modifier
        for line in to_predict.splitlines():
            if i == 0:
                sample_x = read_sample_x_from_string(line, feature_count=config.data_config.input_features,
    ...
    def create_ensemble(self, config: ModelConfig, ensemble_weights_paths: List[str], hf=None):
        ensemble = [Model(config) for _ in range(len(ensemble_weights_paths))]
        for i in range(len(ensemble)):
            if hf:        
                 ensemble[i].load_model_weights(ensemble_weights_paths[i], hf)
            else:
    ...
    def predict_and_output(self, ensemble: List[Model], batch_x, batch_z, clear_text):
        ...
        if self.disambiguate and not self.translate and self.output_all_features:
            ...
            for sample_all_features in batch_all_features:
                ...
                self.prediction += sample_all_features + "\n"
            # sys.stdout.flush()  # à modifier
            return
        ...
        if output_wsd is not None and output_translation is None:
            ...
            for sample_wsd in batch_wsd:
                ...
                self.prediction += sample_wsd + "\n"
        elif output_translation is not None and output_wsd is None:
            batch_translation = Predicter.generate_translation_on_batch(output_translation, ensemble[
                0].config.data_config.output_translation_vocabularies[0][0])
            for sample_translation in batch_translation:
                # sys.stdout.write(sample_translation + "\n")
                self.prediction += sample_translation + "\n"
            ...
            for i in range(len(batch_wsd)):
                self.prediction += batch_wsd[i] + "\n"
                self.prediction += batch_translation[i] + "\n"
    ...
    def generate_all_features_on_sample(output, batch_x, i):
        return " ".join(["/".join([str(torch_argmax(output[k][i][j]).item()) for k in range(len(output))]) for j in
                         range(len(batch_x[0][i]))])
