...
class EmbeddingsAuto(Module):
    ...
    def __init__(self, auto_model: str, auto_path: str):
        ...
        elif "xlm-roberta" in auto_model:
            from transformers import XLMRobertaModel, XLMRobertaTokenizer
            self.auto_embeddings = XLMRobertaModel.from_pretrained(auto_path)
            self.auto_tokenizer = XLMRobertaTokenizer.from_pretrained(auto_path)
    ...
    def get_begin_special_token_count(self):
        ...
        if isinstance(self.auto_tokenizer, XLMTokenizer):
            return 1
        if isinstance(self.auto_tokenizer, XLMRobertaTokenizer):
            return 1
        else:
