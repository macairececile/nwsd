...
class EmbeddingsLUT(Module):
    ...
    def __init__(self, input_embeddings: str, input_vocabulary_size: int, input_embeddings_size: int, clear_text: bool, tokenize_model: str):
        ...
        if clear_text:
            ...
            self.lut_embeddings = Embedding(num_embeddings=input_vocabulary_size, embedding_dim=input_embeddings_size, padding_idx=self.pad_token_index)
    ...
    def preprocess_sample_next(self, sample_x, new_size, indices):
        ...
        if indices is None:
            return sample_x
        new_sample_x = torch_full([new_size], fill_value=self.unk_token_index, dtype=torch_long)
