...
#SBATCH --job-name=camembert_base_ensemble
#SBATCH -A czj@v100
#SBATCH -C v100-32g
#SBATCH --gres=gpu:1
#SBATCH --ntasks=8
...
#SBATCH --output=camembert_base_ensemble
...
TARGET_DIRECTORY=/gpfsscratch/rech/czj/uef37or/wsd/camembert_base_ensemble # to store the trained model
...
mkdir "$dir_model/pictos_cecile"
PICTO_PATH="$dir_model/pictos_cecile"

python src/NeuralWSDDecodePictos_cecile.py \
--data_path $DATA_DIRECTORY \
--weights $TARGET_DIRECTORY/model_weights_wsd0 $TARGET_DIRECTORY/model_weights_wsd1 $TARGET_DIRECTORY/model_weights_wsd2 $TARGET_DIRECTORY/model_weights_wsd3 $TARGET_DIRECTORY/model_weights_wsd4 $TARGET_DIRECTORY/model_weights_wsd5 $TARGET_DIRECTORY/model_weights_wsd6 $TARGET_DIRECTORY/model_weights_wsd7 \
--lowercase True \
--sense_compression_hypernyms False \
--filter_lemma False \
--clear_text False \
--corpus /gpfswork/rech/czj/uef37or/nwsd/data/corpora/corpus_txt_pictos_ids_wn_300_cecile.csv \
--saved_path $PICTO_PATH > $PICTO_PATH/wsd_cecile.txt
